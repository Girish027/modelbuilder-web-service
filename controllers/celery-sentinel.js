...
function RedisBroker(conf) {
    ...
    self.redis = redis.createClient(conf.SENTINEL_BROKER_URLS, conf.MASTER_NAME, conf.OPTS);
...
function RedisBackend(conf) {
   ...
   var backend_ex = redis.createClient(conf.SENTINEL_BROKER_URLS, conf.MASTER_NAME, conf.OPTS) ; //self.redis.duplicate();
